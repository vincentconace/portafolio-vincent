...
/**
 * Isotipo oficial de Mercado Pago (óvalo + apretón de manos), un solo path en
 * `currentColor`. Se pinta en el celeste de marca donde se usa.
 */
function MercadoPago({ className, ...props }) {
  return (
    <svg
      viewBox='0 0 24 24'
      fill='currentColor'
...
/**
 * Card destacada del ebook a la venta: portada real + serie + título + descripción
 * + CTA a Payhip. Toda la card es un único link (tappable entera en mobile); el
 * botón es un <span> visual para no anidar interactivos. Look editorial de la web
 * (tinta monocroma; el único color lo aporta la portada).
 *
 * @param {{ ebook: { id: string; cover: string; href: string } }} props
 */
      ...
      <div className='flex flex-col items-center gap-5 sm:flex-row sm:items-center sm:gap-7'>
        ...
        <div className='min-w-0 flex-1 text-center sm:text-left'>
          <p className='text-[0.68rem] font-medium uppercase tracking-[0.14em] text-muted-foreground'>
            {t('links.ebooksSeries')}
          </p>
          <h2 className='mt-2 text-[clamp(1.4rem,3vw,1.9rem)] font-medium leading-[1.1] tracking-[-0.02em] text-foreground'>
            {t(`${base}.title`)}
          </h2>
          <p className='mx-auto mt-2.5 max-w-[46ch] text-sm leading-relaxed text-muted-foreground sm:mx-0 sm:text-base'>
            {t(`${base}.desc`)}
          </p>
